...
export function recursiveCloneChildren(
	...
	// The previous approach of returning children as-is on server caused button/anchor mismatches
	return React.Children.map(children, (child, index) => {
		...
		const shouldApplyProps = componentNames ? componentNames.includes(displayName || "") : true
