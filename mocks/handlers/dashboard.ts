...
export const dashboardHandlers = [
	...
	http.get(encoreUrl("/organizations/:orgId/dashboard"), async ({ params, request }) => {
		...
		const dashboardStats =
			db.dashboardStats.findFirst((q) => q.where({ organizationId: orgId })) ||
		...
		const enrollmentChart = Array.from({ length: 7 }, (_, i) => {
			...
			const dayEnrollments = recentEnrollments.filter(
				(e) => typeof e.createdAt === "string" && e.createdAt.startsWith(dateStr)
			)
		...
		const pendingEnrollments = orgEnrollments
			...
			.map((e) => {
				...
				const product = campaign
					? db.products.findFirst((q) => q.where({ id: campaign.productId }))
					: null
