...
} from "@/features/campaigns/lib/api"
...
export const deliverableKeys = {
	...
	list: (filters?: { status?: DeliverableStatus }) =>
...
export function useDeliverableTypes(status?: DeliverableStatus) {
	return useQuery({
		...
		queryFn: async () => {
			const result = await listDeliverables({ status })
			return result.data
		},
...
export function useCampaignDeliverables(campaignId: string) {
	return useQuery({
		...
		queryFn: async () => {
			const result = await listCampaignDeliverables(campaignId)
			return result.data
		},
		enabled: !!campaignId,
...
export function useAddCampaignDeliverablesBatch(campaignId: string) {
	...
	return useMutation({
		mutationFn: (deliverables: { deliverableId: string; quantity?: number; payout?: number }[]) =>
			addCampaignDeliverablesBatch(campaignId, { deliverables }),
		onSuccess: () => {
...
export function useUpdateCampaignDeliverable() {
	...
	return useMutation({
		mutationFn: ({
			...
			isRequired?: boolean
			instructions?: string
		}) => updateCampaignDeliverable(id, { quantity, isRequired, instructions }),
		onSuccess: () => {
...
export function useRemoveCampaignDeliverable() {
	...
	return useMutation({
		mutationFn: (id: string) => removeCampaignDeliverable(id),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: deliverableKeys.all })
		},
	})
