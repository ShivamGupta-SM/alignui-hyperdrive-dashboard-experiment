...
export function TeamClient({ initialData }: TeamClientProps = {}) {
	...
	const [dismissedOnboardingAlert, setDismissedOnboardingAlert] = useLocalStorage<boolean>(
		"team-onboarding-alert-dismissed",
		false
	)
	...
	const currentUserId = session?.user ? (("userID" in session.user ? session.user.userID : session.user.id) || "") : ""
					...
					<div className="p-3">
						<div className="space-y-2">
							{invitations.map((invitation) => (
								<div
									key={invitation.id}
									className="flex items-center gap-3 sm:gap-4 p-3 rounded-lg bg-bg-white-0 ring-1 ring-inset ring-stroke-soft-200"
								>
									<AvatarWithFallback
										src={undefined}
					...
					<div className="space-y-2">
						{ROLE_OPTIONS.map((role) => {
							...
							return (
								<div
									key={role.value}
									className="flex items-center gap-3 p-3 rounded-lg bg-bg-weak-50"
								>
									<div
										className={cn(
...
function InviteMemberModal({
	...
	const inviteMemberAction = async (
		prevState: Result<{ invitationId: string }> | null,
		formData: FormData
	): Promise<Result<{ invitationId: string }>> => {
		const email = formData.get("email") as string
		const role = formData.get("role") as string
		if (!email || !role) {
			return { success: false, error: new Error("Email and role are required") }
		}
				...
				<Modal.Body>
					<form id="invite-form" action={formAction}>
						<div className="space-y-4">
										...
										return (
											<label
												...
												className={cn(
													...
													"ring-1 ring-inset ring-stroke-soft-200 hover:bg-bg-weak-50"
												)}
											>
							...
							<div>
								...
								<Textarea.Root
									...
									maxLength={VALIDATION_CONSTANTS.MESSAGE_MAX_LENGTH}
								/>
						...
						<div className="mt-4 flex items-center gap-2 p-3 rounded-10 bg-bg-weak-50">
							...
							<span className="text-paragraph-xs text-text-sub-600">Invite expires in 7 days</span>
						</div>
					</form>
				</Modal.Body>
				<Modal.Footer>
		...
		<Modal.Root open={open} onOpenChange={onOpenChange}>
			<Modal.Content>
				...
				<Modal.Body>
					...
					<div className="text-paragraph-sm text-text-sub-600">This will:</div>
						...
						<List.Item>
							...
							<List.ItemContent>
								<List.ItemDescription>
									Not affect their personal Hypedrive account
								</List.ItemDescription>
							</List.ItemContent>
						</List.Item>
					</List.Root>
				...
				<Modal.Footer>
					<Button.Root variant="ghost" onClick={() => onOpenChange(false)} disabled={isPending}>
						Cancel
					</Button.Root>
