			...
			<div className="rounded-xl bg-bg-white-0 ring-1 ring-inset ring-stroke-soft-200 p-4 sm:p-5">
				<div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
					...
					<div className="min-w-0">
						<h2 className="text-label-md sm:text-label-lg text-text-strong-950 truncate">
							{user.name || "User"}
						</h2>
				...
				<MetricGroup columns={3} className="grid-cols-3">
					...
					<Metric
						...
						size="sm"
						className={cn(!user.twoFactorEnabled && "[&>div>span:last-child]:text-warning-base")}
					/>
				</MetricGroup>
			</div>
	...
	return (
		<div className="space-y-5 sm:space-y-6">
			...
			<div className="rounded-xl bg-bg-white-0 ring-1 ring-inset ring-stroke-soft-200 p-4 sm:p-6">
				<h3 className="text-label-sm sm:text-label-md text-text-strong-950 mb-3 sm:mb-4">
					Profile Photo
				</h3>
			...
			<div className="rounded-xl bg-bg-white-0 ring-1 ring-inset ring-stroke-soft-200 p-4 sm:p-6">
				...
				<div className="space-y-4 max-w-md">
					<div>
						<label className="block text-label-sm text-text-strong-950 mb-2">Full Name</label>
						<Input.Root>
							<Input.Wrapper>
								<Input.El value={name} onChange={(e) => setName(e.target.value)} />
							</Input.Wrapper>
						</Input.Root>
					</div>
					<div>
						<label className="block text-label-sm text-text-strong-950 mb-2">Email Address</label>
						<Input.Root>
							<Input.Wrapper>
								<Input.El type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
							</Input.Wrapper>
						</Input.Root>
		...
		<div className="space-y-6">
			...
			<div className="rounded-xl bg-bg-white-0 ring-1 ring-inset ring-stroke-soft-200 p-6">
				...
				<div className="space-y-4 max-w-md">
					...
					<div>
						<label className="block text-label-sm text-text-strong-950 mb-2">New Password</label>
						<Input.Root>
							<Input.Wrapper>
								<Input.El
									type="password"
				...
				<div className="mt-6 pt-6 border-t border-stroke-soft-200">
					<Button.Root
						...
						disabled={
							!currentPassword || !newPassword || newPassword !== confirmPassword || isSaving
						}
					>
			...
			<div className="rounded-xl bg-bg-white-0 ring-1 ring-inset ring-stroke-soft-200 p-6">
				<List.Root size="lg">
					<List.Item>
						...
						<List.ItemAction>
							<Switch.Root checked={twoFactorEnabled} onCheckedChange={setTwoFactorEnabled} />
						</List.ItemAction>
					</List.Item>
				</List.Root>
				{twoFactorEnabled && (
				...
				<List.Root variant="divided" size="md">
					<List.Item>
						<List.ItemContent>
							...
							<List.ItemDescription>
								Get notified when shoppers enroll in your campaigns
							</List.ItemDescription>
						</List.ItemContent>
						<List.ItemAction>
							<Switch.Root
					...
					<List.Item>
						<List.ItemContent>
							...
							<List.ItemDescription>
								Get notified when your campaigns are approved or rejected
							</List.ItemDescription>
						</List.ItemContent>
						<List.ItemAction>
							<Switch.Root
					...
					<List.Item>
						<List.ItemContent>
							...
							<List.ItemDescription>
								Get notified about wallet transactions and balance changes
							</List.ItemDescription>
						</List.ItemContent>
						<List.ItemAction>
							<Switch.Root
				...
				<List.Root variant="divided" size="md">
					<List.Item>
						<List.ItemContent>
							...
							<List.ItemDescription>
								Get real-time push notifications for important updates
							</List.ItemDescription>
						</List.ItemContent>
						<List.ItemAction>
							<Switch.Root
			...
			<div className="rounded-xl bg-bg-white-0 ring-1 ring-inset ring-stroke-soft-200 p-6">
				...
				<div className="flex items-center gap-2 mt-4 pt-4 border-t border-stroke-soft-200 p-3 rounded-10 bg-warning-lighter">
					...
					<span className="text-paragraph-xs text-warning-dark">
						If you see a device you don't recognize, revoke access immediately and change your
						password.
					</span>
				</div>
			</div>
