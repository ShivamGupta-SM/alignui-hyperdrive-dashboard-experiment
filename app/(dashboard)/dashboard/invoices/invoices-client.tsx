				...
				<div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0 sm:overflow-visible scrollbar-hide">
					<div className="flex gap-2 sm:gap-3 min-w-max sm:min-w-0 sm:grid sm:grid-cols-4">
						{[
							...
							>
								<span className="text-[10px] sm:text-paragraph-xs text-text-soft-400 uppercase tracking-wide">
									{stat.label}
								</span>
								<span className="text-label-md sm:text-label-lg text-text-strong-950 font-semibold mt-0.5">
									{stat.value}
								</span>
							</div>
						))}
...
function InvoiceItem({
	...
	return (
		<>
			...
			>
				<div className="flex items-start justify-between gap-3 mb-3">
					<div className="flex items-center gap-2">
						...
						<div>
							<p className="text-label-sm text-text-strong-950 font-mono">
				...
				<div className="flex items-center justify-between mt-3">
					<div>
						<p className="text-title-h5 text-text-strong-950 font-semibold">
							{formatCurrency(invoice.totalAmount)}
						</p>
						<p className="text-paragraph-xs text-text-soft-400">
							incl. {formatCurrency(invoice.gstAmount)} GST
						</p>
					</div>
			...
			{/* Desktop: Row layout */}
			<div
				className="hidden sm:flex items-center gap-4 p-4 hover:bg-bg-weak-50 transition-all duration-200 cursor-pointer group"
				onClick={onView}
			>
				<div className="size-11 rounded-xl bg-bg-weak-50 flex items-center justify-center shrink-0 transition-colors group-hover:bg-primary-lighter">
					<FileText
						weight="duotone"
						className="size-6 text-text-soft-400 transition-colors group-hover:text-primary-base"
					/>
				</div>
				...
				<div className="text-right shrink-0">
					<p className="text-label-md text-text-strong-950 font-semibold">
						{formatCurrency(invoice.totalAmount)}
					</p>
					<p className="text-paragraph-xs text-text-soft-400">
		...
		<>
			...
			<div className="px-4 py-4 space-y-4">
				...
				<div className="text-center">
					<p className="text-[10px] text-text-soft-400 uppercase tracking-widest mb-0.5">
						Tax Invoice
					</p>
					<p className="text-sm font-semibold text-text-strong-950 font-mono">
						{invoice.invoiceNumber}
					</p>
				</div>
				...
				<div className="grid grid-cols-2 gap-x-4 gap-y-2 py-2.5 px-3 bg-bg-weak-50 rounded-md">
					<div>
						...
						<p className="text-[11px] font-medium text-text-strong-950">
							{formatDate(invoice.createdAt)}
						</p>
					</div>
					<div>
				...
				<div className="border-t border-stroke-soft-200 pt-2 space-y-1.5">
					...
					<div className="flex justify-between pt-2 border-t border-stroke-soft-200">
						...
						<span className="text-sm font-bold text-text-strong-950">
							{formatCurrency(invoice.totalAmount)}
						</span>
					</div>
				</div>
			...
			<div className="px-4 py-3 bg-bg-weak-50 border-t border-stroke-soft-200">
				<div className="flex items-center justify-center gap-3 mb-2 flex-wrap">
					...
					>
						<Button.Icon
							as={isDownloading ? SpinnerGap : DownloadSimple}
							className={isDownloading ? "animate-spin" : ""}
						/>
						Download PDF
					</Button.Root>
					<Button.Root variant="neutral" size="xsmall" onClick={() => window.print()}>
					...
					{invoice.enrollmentCount > 0 && onExportEnrollments && (
						...
						>
							<Button.Icon
								as={isExportingEnrollments ? SpinnerGap : DownloadSimple}
								className={isExportingEnrollments ? "animate-spin" : ""}
							/>
							Export Enrollments
						</Button.Root>
					)}
				...
				<p className="text-[9px] text-text-soft-400 mt-0.5 text-center">
					support@hypedrive.io • hypedrive.io
				</p>
			</div>
		</>
	)
...
function InvoiceModal({
	...
	return (
		<Modal.Root open={!!invoice} onOpenChange={(open) => !open && onClose()}>
			<Modal.Content showClose={false} className="sm:max-w-[380px] p-0 overflow-hidden rounded-xl!">
				{/* Header */}
