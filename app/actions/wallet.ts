...
export async function requestCreditAction(
	...
	const result = await requestCredit({ amount: Number(amount), reason })

	if (result.success) {
		return { success: true, message: result.message || "Credit request submitted successfully" }
	}

	return { success: false, error: result.error || "Failed to submit credit request" }
}
