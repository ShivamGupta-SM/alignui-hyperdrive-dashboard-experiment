...
const ProgressBarRoot = React.forwardRef<HTMLDivElement, ProgressBarRootProps>(
	(
		{ className, color, size, value = 0, max = 100, isIndeterminate = false, ...rest },
		forwardedRef
	) => {
		...
		return (
			...
			>
				{isIndeterminate ? (
					<div className={indeterminate()} style={{ width: "40%" }} />
				) : (
			...
			<div className="flex gap-1">
				{Array.from({ length: steps }).map((_, index) => (
					<div
						...
						aria-valuemax={100}
					>
						<div className={progress()} style={{ width: index < currentStep ? "100%" : "0%" }} />
					</div>
				))}
			</div>
