...
const sliderVariants = tv({
	slots: {
		...
		thumb:
			"box-content block shrink-0 cursor-pointer rounded-full border-static-white bg-primary-base shadow-toggle-switch outline-none focus:outline-none",
	},
	variants: {
		size: {
			sm: {
...
>(({ className, children, size, color, ...rest }, forwardedRef) => {
	...
	return (
		<SliderPrimitive.Root ref={forwardedRef} className={cn(root(), className)} {...rest}>
...
>(({ className, size, color, ...rest }, forwardedRef) => {
	...
	return <SliderPrimitive.Thumb ref={forwardedRef} className={cn(thumb(), className)} {...rest} />
})
			...
			{(label || (showValue && valuePosition === "right")) && (
				<div className="flex items-center justify-between">
					...
					{showValue && valuePosition === "right" && (
						<span className="text-label-sm text-text-sub-600 tabular-nums">{displayValue}</span>
					)}
				</div>
			)}
			<SliderRoot
