...
>(({ className, ...rest }, forwardedRef) => {
	return <thead ref={forwardedRef} className={className} {...rest} />
})
...
const TableBody = React.forwardRef<
	...
	React.HTMLAttributes<HTMLTableSectionElement>
>(({ className, ...rest }, forwardedRef) => {
	return <tbody ref={forwardedRef} className={className} {...rest} />
})
			...
			>
				<span className="flex items-center gap-1">
					...
					<span className="ml-auto shrink-0" aria-hidden="true">
						{sortDirection === "asc" && (
							<ArrowUp className="size-4 text-text-strong-950" weight="bold" />
						)}
						{sortDirection === "desc" && (
							<ArrowDown className="size-4 text-text-strong-950" weight="bold" />
						)}
...
function TableLoading({ rows = 5, columns = 4, className, ...rest }: TableLoadingProps) {
	return (
		<tbody className={className} aria-busy="true" aria-live="polite" {...rest}>
			{Array.from({ length: rows }).map((_, rowIndex) => (
				<tr key={rowIndex} className="group/row">
					{Array.from({ length: columns }).map((_, colIndex) => (
						<td key={colIndex} className="h-16 px-3 first:rounded-l-xl last:rounded-r-xl">
							<Skeleton variant="text" className={colIndex === 0 ? "w-2/3" : "w-full"} />
						</td>
					))}
				</tr>
			))}
