...
function loadCanvasKit(): Promise<unknown> {
	...
	canvasKitPromise = new Promise((resolve, reject) => {
		...
		if ((window as unknown as Record<string, unknown>).CanvasKitInit) {
			const init = (window as unknown as Record<string, unknown>).CanvasKitInit as (opts: {
...
export function SkiaLiquidLogo({
	...
	useEffect(() => {
		...
		async function initSkia() {
			try {
				...
				const surface = (CanvasKit.MakeCanvasSurface as (canvas: HTMLCanvasElement) => unknown)(
					canvas
				)
				if (!surface) {
        ...
        `
				...
				function draw() {
					...
					const skCanvas = (
						ckRef.current.surface as { getCanvas: () => unknown }
					).getCanvas() as Record<string, unknown>
					...
					const logoImg = logoImage as {
						makeShaderOptions: (tm1: unknown, tm2: unknown, fm: unknown, mm: unknown) => unknown
					}
					...
					const shaderObj = shader as {
						makeShaderWithChildren: (uniforms: number[], children: unknown[]) => unknown
					}
					...
					;(skCanvas.drawRect as (rect: unknown, paint: unknown) => void)(
						XYWHRect(0, 0, canvas!.width, canvas!.height),
						paint
					)
	...
	if (error) {
		return (
			<div
				className={`flex items-center justify-center bg-bg-weak-50 text-text-sub-600 text-paragraph-xs p-2 text-center ${className}`}
			>
				Skia: {error}
			</div>
		)
