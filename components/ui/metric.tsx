...
interface MetricProps extends React.HTMLAttributes<HTMLDivElement>, MetricVariantProps {
	label?: string
	value: string | number
	delta?: string | number
	deltaType?: DeltaType
...
function MetricCard({ icon, className, ...rest }: MetricCardProps) {
	return (
		<div className={cn("rounded-xl border border-stroke-soft-200 bg-bg-white-0 p-4", className)}>
...
interface ComparisonMetricProps extends Omit<MetricProps, "delta" | "deltaType" | "value"> {
...
function MetricGroup({ columns = 3, className, children, ...rest }: MetricGroupProps) {
	const gridCols = {
		2: "grid-cols-2",
		3: "grid-cols-3",
		4: "grid-cols-4",
	...
	return (
		<div className={cn("grid gap-4", gridCols[columns], className)} {...rest}>
			{children}
		</div>
	)
}
...
export { Metric, MetricCard, ComparisonMetric, MetricGroup, metricVariants, type DeltaType }
