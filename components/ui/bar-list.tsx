...
function BarList({
	...
	const maxValue = React.useMemo(() => Math.max(...sortedData.map((d) => d.value), 0), [sortedData])
				...
				const content = (
					<>
						<div className={labelWrapper()}>
							{dataItem.icon && <span className="shrink-0 text-text-sub-600">{dataItem.icon}</span>}
...
function BarListWithHeader({ title, subtitle, className, ...rest }: BarListWithHeaderProps) {
	return (
		<div className={cn("flex flex-col gap-4", className)}>
			{(title || subtitle) && (
				<div className="flex flex-col gap-0.5">
					{title && <h3 className="text-label-md text-text-strong-950">{title}</h3>}
					{subtitle && <p className="text-paragraph-sm text-text-sub-600">{subtitle}</p>}
				</div>
			)}
			<BarList {...rest} />
		</div>
...
export { BarList, BarListWithHeader, barListVariants, type BarListItem }
