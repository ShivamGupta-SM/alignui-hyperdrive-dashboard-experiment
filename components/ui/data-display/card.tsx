...
const CardRoot = React.forwardRef<HTMLDivElement, CardRootProps>(
	({ className, variant, size, interactive, ...rest }, forwardedRef) => {
		...
		return <div ref={forwardedRef} className={cn(root(), className)} {...rest} />
	}
)
...
const CardHeader = React.forwardRef<HTMLDivElement, CardHeaderProps>(
	({ className, size = "md", ...rest }, forwardedRef) => {
		...
		return <div ref={forwardedRef} className={cn(header(), className)} {...rest} />
	}
)
...
const CardTitle = React.forwardRef<HTMLHeadingElement, CardTitleProps>(
	({ className, as: Component = "h3", ...rest }, forwardedRef) => {
		...
		return <Component ref={forwardedRef} className={cn(title(), className)} {...rest} />
	}
)
...
>(({ className, ...rest }, forwardedRef) => {
	...
	return <p ref={forwardedRef} className={cn(description(), className)} {...rest} />
})
...
const CardFooter = React.forwardRef<HTMLDivElement, CardFooterProps>(
	({ className, size = "md", align = "right", ...rest }, forwardedRef) => {
		...
		return (
			<div ref={forwardedRef} className={cn(footer(), alignClasses[align], className)} {...rest} />
		)
	}
)
...
function StatCard({ title, value, change, icon, footer, size = "md", ...rest }: StatCardProps) {
	const trendColors = {
