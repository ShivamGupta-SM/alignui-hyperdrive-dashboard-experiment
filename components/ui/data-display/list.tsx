...
interface ListRootProps extends React.HTMLAttributes<HTMLUListElement>, ListVariantProps {}
	...
	({ className, variant, size, interactive, children, ...rest }, forwardedRef) => {
		...
		return (
			<ListContext.Provider value={{ variant, size, interactive }}>
				<ul ref={forwardedRef} className={cn(root(), className)} role="list" {...rest}>
					{children}
				</ul>
...
const ListItem = React.forwardRef<HTMLLIElement, ListItemProps>(
	({ className, children, ...rest }, forwardedRef) => {
		...
		return (
			<li ref={forwardedRef} className={cn(item(), className)} {...rest}>
				{children}
			</li>
		)
	}
...
const ListItemContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
	({ className, ...rest }, forwardedRef) => {
		...
		return <div ref={forwardedRef} className={cn(content(), className)} {...rest} />
	}
)
...
const ListItemTitle = React.forwardRef<HTMLSpanElement, React.HTMLAttributes<HTMLSpanElement>>(
	({ className, ...rest }, forwardedRef) => {
		...
		return <span ref={forwardedRef} className={cn(title(), "truncate", className)} {...rest} />
	}
)
...
>(({ className, ...rest }, forwardedRef) => {
	...
	return <span ref={forwardedRef} className={cn(description(), "truncate", className)} {...rest} />
})
...
const ListItemAction = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
	({ className, ...rest }, forwardedRef) => {
		...
		return <div ref={forwardedRef} className={cn(action(), className)} {...rest} />
	}
)
...
function SimpleListItem({ icon, title, description, action, ...rest }: SimpleListItemProps) {
	return (
		<ListItem {...rest}>
...
>(({ className, variant, size, children, ...rest }, forwardedRef) => {
	...
	return (
		<ListContext.Provider value={{ variant, size }}>
			<ul ref={forwardedRef} className={cn(root(), "list-disc list-inside", className)} {...rest}>
				{children}
			</ul>
		</ListContext.Provider>
	)
