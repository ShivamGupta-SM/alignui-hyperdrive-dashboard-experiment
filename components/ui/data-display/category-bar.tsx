...
function CategoryBar({
	...
	if (total === 0) {
		return (
			<div className={cn(root(), className)} role="img" aria-label="Category bar with no data" />
		)
	}
		...
		if (showTooltip) {
			return (
				<Tooltip.Root key={item.name}>
					<Tooltip.Trigger asChild>{segmentEl}</Tooltip.Trigger>
					<Tooltip.Content>
				...
				<div className="flex flex-wrap gap-x-4 gap-y-1">
					{segments.map((item) => (
						<div key={item.name} className={label()}>
							...
							<span>{item.name}</span>
							<span className="text-text-soft-400">({item.percentage.toFixed(1)}%)</span>
						</div>
					))}
				</div>
			)}
	...
	return (
		<div className={cn("relative", className)}>
			<div className={root()} role="img" aria-label={`Value: ${value} out of ${max}`}>
				<div className={segment()} style={{ width: `${low}%`, backgroundColor: rangeColors[0] }} />
				<div
					className={segment()}
