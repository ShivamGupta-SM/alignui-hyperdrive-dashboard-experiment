...
export const featuredCardVariants = tv({
	slots: {
		...
		progressBar: ["mt-4 h-2 w-full overflow-hidden rounded-full bg-bg-soft-200"],
		progressBarIndicator: ["h-full rounded-full transition-all duration-300 ease-out"],
...
const FeaturedCardProgressBar = React.forwardRef<HTMLDivElement, FeaturedCardProgressBarProps>(
	(
		{
			className,
			color = "primary",
		...
		return (
			<div ref={forwardedRef} className={styles.root({ class: className })} {...rest}>
				{showCloseButton && (
					<button
						type="button"
				...
				<div
					...
					aria-valuemax={100}
				>
					<div className={styles.progressBarIndicator()} style={{ width: `${clampedProgress}%` }} />
				</div>
...
export { FeaturedCard, FeaturedCardProgressBar, FeaturedCardProgressCircle }
