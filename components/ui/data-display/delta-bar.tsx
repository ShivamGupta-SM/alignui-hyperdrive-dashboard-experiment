...
interface DeltaBarProps extends React.HTMLAttributes<HTMLDivElement>, DeltaBarVariantProps {
...
function DeltaBar({
	...
	return (
		...
		>
			<div className={track()}>
				<div
					className={cn(bar(), isPositive ? positiveColor : negativeColor)}
					style={{
			...
			{showMarker && <div className={marker()} style={{ left: "50%", marginLeft: "-1px" }} />}
		</div>
	)
}
...
interface ComparisonDeltaBarProps extends Omit<DeltaBarProps, "value">, DeltaBarVariantProps {
	startValue: number
	endValue: number
	label?: string
	showLabels?: boolean
		...
		<div className={cn("flex flex-col gap-2", className)}>
			...
			{showLabels && (
				<div className="flex items-center justify-between text-paragraph-xs">
					...
					<span className={cn("font-medium", delta >= 0 ? "text-success-base" : "text-error-base")}>
						{delta >= 0 ? "+" : ""}
...
export { DeltaBar, LabeledDeltaBar, ComparisonDeltaBar, deltaBarVariants }
