...
function HorizontalStepperRoot({
	...
	return (
		<Component className={cn("flex flex-wrap justify-center gap-4", className)} {...rest}>
			{children}
		</Component>
	)
}
...
function HorizontalStepperSeparatorIcon<T extends React.ElementType>({
	...
	return <Component className={cn("size-5 shrink-0 text-text-soft-400", className)} {...rest} />
}
...
type HorizontalStepperItemSharedProps = VariantProps<typeof horizontalStepperItemVariants>

type HorizontalStepperItemProps = React.ButtonHTMLAttributes<HTMLButtonElement> &
...
HorizontalStepperItemIndicator.displayName = HORIZONTAL_STEPPER_ITEM_INDICATOR_NAME
