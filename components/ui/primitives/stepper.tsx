...
function StepperItem({ className, step, hasError = false, children, ...rest }: StepperItemProps) {
	...
	const status = hasError ? "error" : isCompleted ? "completed" : isActive ? "active" : "inactive"
	...
	return (
		<div className={item({ class: className })} data-status={status} data-step={step} {...rest}>
			{extendedChildren}
		</div>
	)
}
...
function StepperSeparator({ className, isLast, status, ...rest }: StepperSeparatorProps) {
...
function StepperContent({ className, ...rest }: StepperContentProps) {
	...
	return <div className={cn(content(), "mt-2 text-center", className)} {...rest} />
}
