...
interface TrackerProps extends React.HTMLAttributes<HTMLDivElement>, TrackerVariantProps {
	data: TrackerBlock[]
}
...
function LabeledTracker({ label, value, data, size, className, ...rest }: LabeledTrackerProps) {
	return (
		<div className={cn("flex flex-col gap-2", className)}>
			{(label || value !== undefined) && (
				<div className="flex items-center justify-between">
					{label && <span className="text-label-sm text-text-strong-950">{label}</span>}
					{value !== undefined && <span className="text-label-sm text-text-sub-600">{value}</span>}
				</div>
			)}
			<Tracker data={data} size={size} {...rest} />
		</div>
