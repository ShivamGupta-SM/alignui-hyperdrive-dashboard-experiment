...
interface LegendProps extends React.HTMLAttributes<HTMLDivElement>, LegendVariantProps {
...
function Legend({
	...
	const { root, item: itemClass, indicator, label } = legendVariants({ size, layout })
		...
		<div className={cn(root(), className)} role="list" aria-label="Legend" {...rest}>
			{normalizedCategories.map((category, index) => {
				...
				return (
					...
					>
						<span
							className={cn(indicator(), bgColor.startsWith("bg-") ? bgColor : "")}
...
function ColorSwatch({ color, size = "md", className, ...rest }: ColorSwatchProps) {
	...
	return (
		<span
			className={cn(indicator(), bgColor.startsWith("bg-") ? bgColor : "", className)}
			style={!bgColor.startsWith("bg-") ? { backgroundColor: bgColor } : undefined}
			{...rest}
		/>
	)
