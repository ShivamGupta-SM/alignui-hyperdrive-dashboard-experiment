...
>(({ className, children, showHandle = true, showClose = true, ...props }, ref) => (
	<BottomSheetPortal>
		...
		<Drawer.Content
			ref={ref}
			className={cn(
				"fixed inset-x-0 bottom-0 z-50 mt-24 flex flex-col rounded-t-2xl bg-bg-white-0",
			...
			style={{
				maxHeight: "calc(100dvh - 6rem - env(safe-area-inset-bottom, 0px))",
			}}
			{...props}
		>
			{showHandle && (
				<div className="mx-auto mt-3 h-1 w-10 shrink-0 rounded-full bg-stroke-soft-200" />
			)}
			{showClose && (
...
>(({ className, ...props }, ref) => (
	<p ref={ref} className={cn("text-paragraph-sm text-text-sub-600", className)} {...props} />
))
...
function BottomSheetFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
	return (
		<div
			className={cn(
				"flex flex-col gap-2 border-t border-stroke-soft-200 px-4 py-4 shrink-0",
				className
			)}
			{...props}
		/>
