...
const modalVariants = tv({
	slots: {
		overlay: [
			"fixed inset-0 z-50 flex items-center justify-center bg-overlay backdrop-blur-[10px]",
			"p-4 overflow-y-auto",
	...
	variants: {
		variant: {
			...
			fullscreen: {
				overlay: "p-0",
				content: [
					"h-full max-w-none rounded-none max-h-full",
	...
	(
		...
		return (
			<ModalContext.Provider
				value={{ titleId, descriptionId, hasTitleBeenRendered, setHasTitleBeenRendered }}
			>
				<ModalPortal>
					<ModalOverlay className={overlayClassName} variant={variant}>
						<DialogPrimitive.Content
							...
							style={{
								maxHeight: variant === "fullscreen" ? "100%" : "calc(100dvh - 2rem)",
							}}
							aria-modal="true"
							{...rest}
						>
...
function ModalHeader({
	...
	return (
		<div
			className={cn(
				"relative flex items-start gap-3.5 py-4 pl-5 pr-14 shrink-0",
...
function ModalBody({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) {
	return (
		<div
			className={cn(
				"p-5 flex-1 overflow-y-auto min-h-0",
