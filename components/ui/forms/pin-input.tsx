...
const PinInputRoot = React.forwardRef<HTMLDivElement, PinInputRootProps>(
	(
		{ className, size = "medium", hasError = false, disabled = false, children, ...rest },
		forwardedRef
	) => {
		...
		return (
			<PinInputContext.Provider value={{ size, hasError, disabled, id }}>
				<div ref={forwardedRef} role="group" className={root({ class: className })} {...rest}>
					{children}
				</div>
...
const PinInputGroup = React.forwardRef<React.ComponentRef<typeof OTPInput>, PinInputGroupProps>(
	(
...
function FakeCaret() {
	return <div className="pointer-events-none h-[1em] w-0.5 animate-caret-blink bg-primary-base" />
}
...
function PinInputSeparator({ className, children, ...rest }: PinInputSeparatorProps) {
	...
	return (
		<div role="separator" className={separator({ class: className })} {...rest}>
			{children ?? "-"}
		</div>
	)
}
