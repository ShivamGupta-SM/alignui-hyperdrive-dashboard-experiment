...
const SearchInput = React.forwardRef<HTMLInputElement, SearchInputProps>(
	(
		{ className, size, hasError, onClear, value, showClearButton = true, ...rest },
		forwardedRef
	) => {
		const { root, wrapper, input, icon } = inputVariants({
			size,
				...
				<label className={wrapper()}>
					{showStepper && (
						<button
							...
							className={icon({
								class:
									"cursor-pointer hover:text-text-strong-950 disabled:cursor-not-allowed disabled:text-text-disabled-300",
							})}
