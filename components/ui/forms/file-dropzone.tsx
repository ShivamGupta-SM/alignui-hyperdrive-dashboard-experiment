			...
			>
				...
				<div className="flex flex-col items-center gap-2 text-center">
					...
					<div>
						...
						<p className="text-paragraph-xs text-text-sub-600">or click to browse</p>
					</div>
			...
			{errors.length > 0 && (
				<div className="space-y-1">
					{errors.map((error, index) => (
						<p key={`error-${error}-${index}`} className="text-paragraph-xs text-error-base">
							{error}
						</p>
					))}
				</div>
			...
			{files.length > 0 && (
				<div className="space-y-2">
					{files.map((file, index) => (
						<div
							key={file.name ? `file-${file.name}-${file.size || index}` : `file-${index}-${file.size || 0}`}
						...
						>
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-paragraph-xs text-text-sub-600">{formatBytes(file.size)}</p>
							</div>
							<Button.Root variant="ghost" size="xsmall" onClick={() => removeFile(index)}>
