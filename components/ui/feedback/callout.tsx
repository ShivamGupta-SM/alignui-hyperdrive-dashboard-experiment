...
const calloutVariants = tv({
	slots: {
		...
		close:
			"absolute right-3 top-3 shrink-0 cursor-pointer opacity-50 transition-opacity hover:opacity-100",
	},
	variants: {
		variant: {
			info: {
...
interface CalloutProps extends React.HTMLAttributes<HTMLDivElement>, CalloutVariantProps {
	title?: string
...
function Callout({
	...
	return (
		...
		>
			...
			{dismissible && (
				<button type="button" onClick={handleDismiss} className={close()} aria-label="Dismiss">
...
function CalloutWithActions({ actions, children, ...rest }: CalloutWithActionsProps) {
	return (
		<Callout {...rest}>
			{children}
...
function QuoteCallout({ children, author, className, ...rest }: QuoteCalloutProps) {
	return (
		<blockquote
			className={cn(
		...
		>
			...
			{author && <footer className="mt-2 text-label-xs text-text-sub-600">— {author}</footer>}
		</blockquote>
	)
}
QuoteCallout.displayName = "QuoteCallout"

export { Callout, CalloutWithActions, InlineCallout, QuoteCallout, calloutVariants }
