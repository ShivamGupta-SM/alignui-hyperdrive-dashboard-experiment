// AlignUI Alert v0.1.0 - Enhanced with dismissible and auto-dismiss
...
const AlertRoot = React.forwardRef<HTMLDivElement, AlertProps>(
	({ children, className, wrapperClassName, size, variant, status, ...rest }, forwardedRef) => {
...
function DismissibleAlert({
	...
	const uniqueId = React.useId()
	const { root, wrapper, closeIcon } = alertVariants({ size, variant, status })
	...
	return (
		<div
			...
			{...rest}
		>
			<div className={cn(wrapper({ class: wrapperClassName }), "relative")}>
				{extendedChildren}
				{showCloseButton && (
					<button
						type="button"
						...
						className={cn(
							closeIcon(),
							"absolute right-0 top-0 cursor-pointer transition-opacity hover:opacity-100"
						)}
...
function AlertWithAction({
	...
	if (actionPosition === "bottom") {
		return (
			<div className={root({ class: className })} role="alert" {...rest}>
	...
	return (
		<div className={root({ class: className })} role="alert" {...rest}>
			<div className={cn(wrapper({ class: wrapperClassName }), "items-center")}>
				<div className="flex-1">{extendedChildren}</div>
