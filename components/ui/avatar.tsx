...
import { IconEmptyCompany, IconEmptyUser } from "@/components/ui/avatar-empty-icons"
...
export const avatarVariants = tv({
	slots: {
		...
		indicator: "absolute flex size-8 items-center justify-center drop-shadow-xs",
	},
	variants: {
		size: {
			"80": {
...
const AvatarRoot = React.forwardRef<HTMLDivElement, AvatarRootProps>(
	(
		{ asChild, children, size, color, className, placeholderType = "user", ...rest },
		forwardedRef
	) => {
...
function AvatarStatus({
	...
	className,
	...rest
}: VariantProps<typeof avatarStatusVariants> & React.HTMLAttributes<HTMLDivElement>) {
	return <div className={avatarStatusVariants({ status, class: className })} {...rest} />
}
...
const AvatarBrandLogo = React.forwardRef<HTMLImageElement, AvatarBrandLogoProps>(
		...
		if (src && !hasError) {
			return (
				<AvatarRoot ref={forwardedRef} size={size} color={color} {...rest}>
					<AvatarImage src={src} alt={alt || name} onError={() => setHasError(true)} />
				</AvatarRoot>
			)
		}
