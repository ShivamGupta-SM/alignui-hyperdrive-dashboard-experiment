		...
		switch (campaign.status) {
			case "draft":
				...
				if (onSubmitForApproval)
					actions.push({ label: "Submit for Approval", icon: Check, onClick: onSubmitForApproval })
				if (onDelete)
					actions.push({ label: "Delete", icon: X, onClick: onDelete, destructive: true })
				break
			...
			case "approved":
				...
				if (onCancel)
					actions.push({ label: "Cancel", icon: X, onClick: onCancel, destructive: true })
				break
			case "active":
						...
						<div className="flex items-start justify-between gap-2">
							<div className="min-w-0">
								...
								<div className="flex items-center gap-2 mt-1.5">
									<Badge.Root color={getStatusBadgeColor()} variant="lighter" size="small">
										{statusConfig.label}
									</Badge.Root>
				...
				<div className="px-4 pb-4">
					<div className="grid grid-cols-3 gap-2 p-3 rounded-xl bg-bg-weak-50">
						...
						<div className="text-center border-x border-stroke-soft-200">
							<div className="text-title-h6 text-warning-base font-semibold">{pendingCount}</div>
							<div className="text-label-xs text-text-soft-400 uppercase tracking-wide">
								Pending
							</div>
						</div>
						<div className="text-center">
							<div className="text-title-h6 text-success-base font-semibold">{approvedCount}</div>
							<div className="text-label-xs text-text-soft-400 uppercase tracking-wide">
								Approved
							</div>
						</div>
					...
					{showProgress && (
						<div className="mt-3">
							<div className="flex items-center justify-between mb-1.5">
								...
								<span className="text-label-xs text-text-strong-950 font-medium">
									{currentEnrollments}/{campaign.maxEnrollments}
								</span>
							</div>
					...
					<div className="flex items-center gap-3 p-3 rounded-xl bg-bg-weak-50">
						<div className="flex items-center gap-2">
							...
							<span className="text-paragraph-sm text-text-sub-600">
								<span className="font-medium text-text-strong-950">{campaign.maxEnrollments}</span>{" "}
								max enrollments
							</span>
						</div>
					</div>
...
export const CampaignListItem = React.memo(function CampaignListItem({
	...
	return (
		<div className="flex items-center gap-3 py-3 border-b border-stroke-soft-200 last:border-0">
			...
			<div className="flex-1 min-w-0">
				<div className="flex items-center gap-2">
					<span className="text-label-sm text-text-strong-950 truncate">{campaign.title}</span>
					<Badge.Root
				...
				<div className="text-paragraph-xs text-text-sub-600 mt-0.5">
					{"currentEnrollments" in campaign ? campaign.currentEnrollments : 0} enrollments ·{" "}
					{"pendingCount" in campaign ? campaign.pendingCount : 0} pending
				</div>
			</div>
