...
import { cn, formatDateShort, formatTimeAgoShort } from "@/lib/utils"
...
function TimelineItem({
	item,
	isLast,
}: { item: EnrollmentTransitionHistoryItem; isLast: boolean }) {
	const config = statusConfig[item.toStatus] || {
		icon: ArrowRight,
	...
	return (
		<div className="relative flex gap-3">
			{/* Timeline line */}
			{!isLast && <div className="absolute left-4 top-8 bottom-0 w-px bg-stroke-soft-200" />}
			...
			<div className="flex-1 pb-4">
				<div className="flex items-start justify-between gap-2">
					<div>
						<p className="text-label-sm text-text-strong-950">{getStatusLabel(item.toStatus)}</p>
						{item.reason && (
							<p className="text-paragraph-xs text-text-sub-600 mt-0.5">{item.reason}</p>
						)}
					</div>
				...
				<div className="flex items-center gap-1.5 mt-1.5">
					...
					<span className="text-paragraph-xs text-text-soft-400">{item.triggeredByName}</span>
				</div>
			</div>
		</div>
	)
	...
	if (!mappedHistory.length) {
		return (
			<div className={cn("text-center py-8", className)}>
				...
				<p className="text-paragraph-sm text-text-soft-400">No history available</p>
			</div>
		)
	}
		...
		<div className={cn("space-y-0", className)}>
			...
			<div>
				{mappedHistory.map((item, index) => (
					<TimelineItem key={item.id} item={item} isLast={index === mappedHistory.length - 1} />
				))}
			</div>
