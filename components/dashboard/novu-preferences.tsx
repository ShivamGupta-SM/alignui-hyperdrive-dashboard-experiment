			...
			<div className="rounded-2xl border border-stroke-soft-200 bg-bg-white-0 p-5">
				...
				<div className="space-y-3">
					{[1, 2, 3].map((i) => (
						<div
							key={i}
							className="flex items-center justify-between p-3 rounded-lg bg-bg-weak-50/50 animate-pulse"
						>
					...
					{Object.entries(globalPrefs.channels).map(([channel, isEnabled]) => {
						...
						const enabled =
							typeof isEnabled === "boolean"
								? isEnabled
								: ((isEnabled as { enabled?: boolean })?.enabled ?? false)
						...
						return (
							...
							>
								<div className="flex items-center gap-3 min-w-0">
									...
									>
										<Icon
											className={cn("size-4", enabled ? "text-primary-base" : "text-text-soft-400")}
											weight="duotone"
										/>
									</div>
...
interface WorkflowPreferenceCardProps {
	preference: ReturnType<typeof usePreferences>["preferences"] extends (infer T)[] | undefined
		? T
		: never
...
function WorkflowPreferenceCard({ preference, updating, onToggle }: WorkflowPreferenceCardProps) {
	...
	const enabledCount = preference?.channels
		? Object.values(preference.channels).filter((c) => {
				const enabled =
					typeof c === "boolean" ? c : ((c as { enabled?: boolean })?.enabled ?? false)
				return enabled
			}).length
		: 0
			...
			>
				<div className="flex items-center gap-3 min-w-0">
					...
					<div className="text-left min-w-0">
						...
						<p className="text-paragraph-xs text-text-sub-600">{enabledCount} channels enabled</p>
					</div>
				</div>
				<svg
					className={cn(
			...
			{expanded && preference?.channels && (
				<div className="px-3 pb-3 space-y-1 border-t border-stroke-soft-200 pt-2">
					{Object.entries(preference.channels).map(([channel, isEnabled]) => {
						...
						return (
							<div key={channel} className="flex items-center justify-between gap-3 p-2 rounded-lg">
