...
const getAvatarColor = (name: string): "blue" | "purple" | "sky" | "yellow" | "red" => {
	const colors: ("blue" | "purple" | "sky" | "yellow" | "red")[] = [
		"blue",
		"purple",
		"sky",
		"yellow",
				...
				<div className="flex-1 min-w-0">
					...
					{enrollment.shopper && (
						<div className="text-paragraph-xs text-text-soft-400 mt-2">
							Previous: {enrollment.shopper.previousEnrollments} • Approval rate:{" "}
							{enrollment.shopper.approvalRate}%
						</div>
					)}
				</div>
				...
				<div className="mt-4 pt-4 border-t border-stroke-soft-200">
					<div className="flex items-center justify-between text-paragraph-sm">
						...
						<span className="text-label-sm text-text-strong-950">
							{formatCurrency(
								Math.round(enrollment.orderValue * (enrollment.lockedBillRate / 100) * 1.18)
							)}
						</span>
					</div>
				</div>
...
export function EnrollmentTableRow({
	...
	return (
		...
		>
			...
			<td className="py-3 px-4">
				<div className="flex items-center gap-3">
					{enrollment.shopper?.avatarUrl ? (
						<Avatar.Root size="32">
							<Avatar.Image
								src={enrollment.shopper.avatarUrl}
								alt={enrollment.shopper?.displayName || "Unknown"}
							/>
						</Avatar.Root>
					) : (
			...
			<td className="py-3 px-4">
				<span className="font-mono text-paragraph-sm text-text-sub-600">{enrollment.orderId}</span>
			</td>
			<td className="py-3 px-4">
			...
			<td className="py-3 px-4">
				{isReviewable ? (
					<Button.Root variant="primary" size="xsmall" onClick={handleReview}>
						Review
					</Button.Root>
				) : (
					<Button.Root variant="neutral" size="xsmall" onClick={handleView}>
						View
					</Button.Root>
				)}
			</td>
