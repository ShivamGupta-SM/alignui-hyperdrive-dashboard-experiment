...
export function MSWDevTools() {
	...
	const fetchDbStats = useCallback(async () => {
		try {
			...
			const mswDb = (window as Window & { __MSW_DB__?: { getStats: () => Promise<DbStats> } })
				.__MSW_DB__
			if (mswDb?.getStats) {
	...
	const handleNetworkChange = useCallback((preset: string) => {
		...
		const mswNetwork = (window as Window & { __MSW_NETWORK__?: { setPreset: (p: string) => void } })
			.__MSW_NETWORK__
	...
	const handleResetDb = useCallback(async () => {
		...
		try {
			const mswDb = (
				window as Window & { __MSW_DB__?: { reset: (scenario: string) => Promise<void> } }
			).__MSW_DB__
			if (mswDb?.reset) {
	...
	useEffect(() => {
		...
		return () => {
			window.removeEventListener(
				"msw:request" as keyof WindowEventMap,
						...
						{activeTab === "database" && (
							<div className="space-y-4">
								...
								{dbStats ? (
									...
									<div className="text-center py-4 text-text-soft-400">Loading stats...</div>
								)}
										...
										{["empty", "minimal", "full"].map((scenario) => (
											<button
												...
												onClick={async () => {
													...
													const mswDb = (
														window as Window & {
															__MSW_DB__?: { reset: (s: string) => Promise<void> }
														}
													).__MSW_DB__
													if (mswDb?.reset) {
						...
						{activeTab === "requests" && (
							<div className="space-y-2">
								{requestLogs.length === 0 ? (
									<div className="text-center py-8 text-text-soft-400">No requests logged yet</div>
								) : (
									requestLogs.map((log) => (
										<div
											key={log.id}
										...
										>
											<div className="flex items-center gap-2">
												...
												<span className="text-text-soft-400">{log.duration}ms</span>
											</div>
