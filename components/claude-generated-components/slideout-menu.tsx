...
export const slideoutMenuVariants = tv({
	slots: {
		...
		header: ["flex items-center gap-3 border-b border-stroke-soft-200 p-5"],
		title: "flex-1 text-label-lg text-text-strong-950",
		description: "text-paragraph-sm text-text-sub-600",
		body: "flex-1 overflow-y-auto",
		footer: ["flex items-center gap-4 border-t border-stroke-soft-200 p-5"],
	},
	variants: {
		side: {
			right: {
...
const SlideoutMenuRoot = ({
	side = "right",
	...props
}: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Root> & SlideoutMenuContextType) => {
	return (
		...
		<div className={header({ class: className })} {...rest}>
			...
			{showCloseButton && (
				<SlideoutMenuClose asChild>
					<CompactButton.Root variant="ghost" size="large" aria-label="Close slideout menu">
...
>(({ className, ...rest }, forwardedRef) => {
	...
	return (
		<DialogPrimitive.Title ref={forwardedRef} className={title({ class: className })} {...rest} />
	)
})
...
function SlideoutMenuBody({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) {
...
function SlideoutMenuFooter({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) {
