...
const Skeleton = React.forwardRef<HTMLDivElement, SkeletonProps>(
	(
		{ className, variant = "rectangular", animation = "shimmer", width, height, style, ...rest },
		forwardedRef
	) => {
		return (
			<div
...
function SkeletonAvatar({ size = "40", className, animation, ...rest }: SkeletonAvatarProps) {
	const sizeMap = {
		"20": "size-5",
		"24": "size-6",
		"32": "size-8",
...
function SkeletonButton({ size = "medium", className, animation, ...rest }: SkeletonButtonProps) {
	const sizeClasses = {
		small: "h-8 w-20",
		medium: "h-9 w-24",
		large: "h-10 w-28",
