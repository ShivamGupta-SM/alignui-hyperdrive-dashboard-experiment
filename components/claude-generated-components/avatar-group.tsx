...
const AvatarGroupRoot = React.forwardRef<HTMLDivElement, AvatarGroupRootProps>(
	({ className, size = "40", direction = "left", max, children, ...rest }, forwardedRef) => {
...
function AvatarGroupOverflow({ className, count, ...rest }: AvatarGroupOverflowProps) {
	...
	return (
		<div className={overflow({ class: className })} aria-label={`${count} more avatars`} {...rest}>
			{displayCount}
		</div>
	)
}
...
export { AvatarGroupRoot as Root, AvatarGroupOverflow as Overflow, AvatarGroupItem as Item }
