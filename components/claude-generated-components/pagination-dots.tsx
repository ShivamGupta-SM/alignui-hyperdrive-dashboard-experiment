...
export const paginationDotsVariants = tv({
	...
	variants: {
		...
		variant: {
			...
			pill: {
				dot: ["bg-bg-soft-200 data-[active=true]:bg-primary-base", "data-[active=true]:w-6"],
			},
		},
	},
	defaultVariants: {
...
export const usePaginationDotsContext = () => React.useContext(PaginationDotsContext)
...
const PaginationDotsRoot = React.forwardRef<HTMLDivElement, PaginationDotsRootProps>(
	(
		{
			className,
			size = "medium",
		...
		return (
			<PaginationDotsContext.Provider value={{ size, variant, activeIndex, onDotClick }}>
				<nav
					ref={forwardedRef}
...
type PaginationDotProps = Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, "onClick"> & {
	index: number
}
...
type SimplePaginationDotsProps = Omit<PaginationDotsRootProps, "children">

function PaginationDots(props: SimplePaginationDotsProps) {
	return <PaginationDotsRoot {...props} />
}

export { PaginationDotsRoot as Root, PaginationDot as Dot, PaginationDots }
